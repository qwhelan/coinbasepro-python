...
class TestAuthenticatedClientSyntax(object):
    def test_place_order_input_1(self, dc):
        with pytest.raises(ValueError):
            r = dc.place_order(
                "BTC-USD", "buy", "market", overdraft_enabled="true", funding_amount=10
            )

    def test_place_order_input_2(self, dc):
        with pytest.raises(ValueError):
            r = dc.place_order(
                "BTC-USD", "buy", "limit", cancel_after="123", time_in_force="ABC"
            )

    def test_place_order_input_3(self, dc):
        with pytest.raises(ValueError):
            r = dc.place_order(
                "BTC-USD", "buy", "limit", post_only="true", time_in_force="FOK"
            )
...
def client():
    ...
    account_info = [x for x in coinbase_accounts if x["name"] == "USD Wallet"][0]
...
class TestAuthenticatedClient(object):
    ...
    def test_place_order(self, client):
        r = client.place_order("BTC-USD", "buy", "limit", price=0.62, size=0.0144)
        assert type(r) is dict
        assert r["stp"] == "dc"
